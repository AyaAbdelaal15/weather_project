...
async function getWeatherData(city) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=9cf1993fede14b16a41121640240907&q=${city}&days=3`,
  );
...
function todayData(data) {
  todayName.innerHTML = new Date(
    data.forecast.forecastday[0].date,
  ).toLocaleString("en-us", { weekday: "long" });
  todayNumber.innerHTML = new Date().getDate();
  todayMonth.innerHTML = new Date(
    data.forecast.forecastday[0].date,
